...
const SubLayer = ({theme, isShow, onClose}) => {
    ...
    useEffect(() => {
        ...
        if (theme.menu && theme.menu.length > 0) {
            ...
            handleLiClick(new Event('click'), firstMenu.src, firstMenu.type);
    ...
    const handleLiClick = async (event, src, type) => {
        ...
        if ( type === 'list' ) { // 소장품 목록일 경우 > 시트데이터를 가져옴
    ...
    const renderPreviewLayer = (data) => {        
        if (!data) return null;
        return <PreviewLayer data={data} isPreviewShow={isPreviewShow} setIsPreviewShow={setIsPreviewShow}/>;        
    };
